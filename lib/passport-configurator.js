...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  if (authType === 'local') {
    ...
    self.app.get(authPath, options.linkHandler || passport.authorize(name, _.defaults({
      scope: scope,
      session: session
    }, options.authOptions)));
  } else {
    self.app.get(authPath, options.authHandler || passport.authenticate(name, _.defaults({
      scope: scope,
      session: session
    }, options.authOptions)));
  }
